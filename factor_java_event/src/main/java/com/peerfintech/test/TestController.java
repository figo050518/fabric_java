...
//            prop2.setProperty("pemFile", peerCert2.getAbsolutePath());
//            Peer peer2 = fabClient.getInstance().newPeer(Config.ORG2_PEER_0, Config.ORG2_PEER_0_URL,prop2);
            String res =  FBUtil.fb.query(Config.CHANNEL_NAME,"factor","QueryDataByFabricTxId",new String[] {fabrictxid.toString()});
            System.out.println("查询结果"+res);
            return "查询结果"+res;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    String add(@RequestBody List<Factor> factor){
        try {
            ...
            for (Factor f:factor
                 ...
                 res =  FBUtil.fb.invoke(Config.CHANNEL_NAME,"factor","SaveData", new String[]{JSONArray.toJSONString(f)});
